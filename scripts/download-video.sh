...
initialize_validate() {
  ...
  mkdir -p "$DOWNLOADS_DIR" || {
    echo "Failed to create downloads directory" | tee -a "$LOG_FILE"
    exit 1
  }
  mkdir -p "$CONFIG_DIR" || {
    echo "Failed to create config directory" | tee -a "$LOG_FILE"
    exit 1
  }

  # Initialize files
  touch "$QUEUE_FILE" "$LOCK_FILE" "$HISTORY_LOG" || {
    echo "Failed to initialize files" | tee -a "$LOG_FILE"
    exit 1
  }
  ...
  echo "$URL" >>"$QUEUE_FILE" || {
    echo "Failed to update queue" | tee -a "$LOG_FILE"
    exit 1
  }
}
...
process_queue() {
  while read -r QUEUED_URL; do
    ...
    echo "$QUEUED_URL" >>"$LOCK_FILE" || {
      echo "Failed to update lock file" | tee -a "$LOG_FILE"
      continue
    }
